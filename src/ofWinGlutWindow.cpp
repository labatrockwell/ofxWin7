...
// hack :/
void HandleFilesW7(WPARAM wParam)
{
...
static LRESULT CALLBACK win7Proc(HWND hwnd, UINT Msg, WPARAM wParam, LPARAM lParam){
...
static void fixCloseWindowOnWin32(){
	...
	SetWindowLongPtr(handle, GWL_WNDPROC, (long)win7Proc);
}
...
//----------------------------------------------------------
ofWinGlutWindow::ofWinGlutWindow(){
	timeNow				= 0;
	timeThen			= 0;
    ...
    touchEnginePtr		= new ofWinTouchEngine(0);
}
...
//------------------------------------------------------------
void ofWinGlutWindow::setGlutDisplayString(string displayStr){
	displayString = displayStr;
}


void ofWinGlutWindow::setDoubleBuffering(bool _bDoubleBuffered){
...
void ofWinGlutWindow::initializeWindow(){
	...
	// don't call this if using gestures, as calling RegisterTouchWindow will
	// stop sending WM_GESTURE events
	if ( !bUseGestures && !bTouchSetup ){
		bTouchSetup = true;
		// test for touch
...
//------------------------------------------------------------
void ofWinGlutWindow::runAppViaInfiniteLoop(ofBaseApp * appPtr){
	ofAppPtr = appPtr;
...
//------------------------------------------------------------
void ofWinGlutWindow::setWindowTitle(string title){
...
//------------------------------------------------------------
ofPoint ofWinGlutWindow::getWindowPosition(){
...
//------------------------------------------------------------
ofPoint ofWinGlutWindow::getScreenSize(){
...
//------------------------------------------------------------
void ofWinGlutWindow::setOrientation(ofOrientation orientationIn){
	orientation = orientationIn;
}

//------------------------------------------------------------
ofOrientation ofWinGlutWindow::getOrientation(){
	return orientation;
}

//------------------------------------------------------------
void ofWinGlutWindow::setWindowPosition(int x, int y){
	glutPositionWindow(x,y);
}

//------------------------------------------------------------
void ofWinGlutWindow::setWindowShape(int w, int h){
...
//------------------------------------------------------------
void ofWinGlutWindow::hideCursor(){
...
//------------------------------------------------------------
void ofWinGlutWindow::showCursor(){
#if defined(TARGET_OSX) && defined(MAC_OS_X_VERSION_10_7)
...
//------------------------------------------------------------
void ofWinGlutWindow::enableSetupScreen(){
	bEnableSetupScreen = true;
}

//------------------------------------------------------------
void ofWinGlutWindow::disableSetupScreen(){
	bEnableSetupScreen = false;
}

//------------------------------------------------------------
void ofWinGlutWindow::setUseGestures( bool useGestures ){
	bUseGestures = useGestures;
	if ( bTouchSetup ){
		bTouchSetup = false;
		UnregisterTouchWindow(handle);
	} else if ( !bUseGestures && !bTouchSetup){
		bTouchSetup = true;
		// test for touch
		int value = GetSystemMetrics(SM_DIGITIZER);
		if (value & NID_READY){ /* stack ready */}
...
//------------------------------------------------------------
void rotateMouseXYW7(ofOrientation orientation, int &x, int &y) {
	int savedY;
	switch(orientation) {
...
void ofWinGlutWindow::mouse_cb(int button, int state, int x, int y) {
	rotateMouseXYW7(orientation, x, y);
...
//------------------------------------------------------------
void ofWinGlutWindow::keyboard_cb(unsigned char key, int x, int y) {
	ofNotifyKeyPressed(key);
}

//------------------------------------------------------------
void ofWinGlutWindow::keyboard_up_cb(unsigned char key, int x, int y){
	ofNotifyKeyReleased(key);
}

//------------------------------------------------------
void ofWinGlutWindow::special_key_cb(int key, int x, int y) {
	ofNotifyKeyPressed(key | OF_KEY_MODIFIER);
}

//------------------------------------------------------------
void ofWinGlutWindow::special_key_up_cb(int key, int x, int y) {
